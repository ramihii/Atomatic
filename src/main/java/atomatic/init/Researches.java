...
public class Researches
{
    ...
    private static void initQuantaResearch()
    {
        ...
        new ResearchItem(PRIM_AIR, CATEGORY_QUANTA, new AspectList().add(Aspect.AIR, 10).add(Aspect.MAGIC, 2), 11, -3, 2, ResourceLocationHelper.getResourceLocation(ICON_LOCATION + PRIM_AIR.toLowerCase() + FILE_EXTENSION)).setParents(WEAK_PRIM_AIR).setParentsHidden(WEAK_PRIM_FIRE, WEAK_PRIM_WATER, WEAK_PRIM_EARTH, WEAK_PRIM_ORDER, WEAK_PRIM_ENTROPY, "ESSENTIACRYSTAL").setConcealed().setPages(new ResearchPage(PAGE_PREFIX + PRIM_AIR + ".1"), new ResearchPage((InfusionRecipe) recipes.get(PRIM_AIR))).registerResearchItem();
    }
}

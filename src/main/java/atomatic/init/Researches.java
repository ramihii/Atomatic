...
public class Researches
{
    ...
    {
        new ResearchItem(INVERSE_DIRT, CATEGORY_QUANTA, new AspectList().add(Aspect.WATER, 1), -4, -2, 1, new ItemStack(ModBlocks.inverseDirt)).setPages(new ResearchPage(PAGE_PREFIX + INVERSE_DIRT + ".1"), new ResearchPage((CrucibleRecipe) recipes.get(INVERSE_DIRT))).registerResearchItem();
        ...
        new ResearchItem(ATOM, CATEGORY_QUANTA, new AspectList().add(Aspect.ENTROPY, 2), 0, 0, 2, ResourceLocationHelper.getResourceLocation("textures/misc/r_atom.png")).setParents(SIZE_SUSPICION).setSpecial().setRound().setPages(new ResearchPage(PAGE_PREFIX + ATOM + ".1"), new ResearchPage(PAGE_PREFIX + ATOM + ".2"), new ResearchPage(QUANTUM, PAGE_PREFIX + ATOM + ".3")).registerResearchItem();
